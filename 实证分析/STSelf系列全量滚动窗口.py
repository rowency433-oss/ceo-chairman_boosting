...
SEED = 42
TRAIN_YEARS = list(range(2008, 2024))  # 2008~2023
TEST_YEARS = [y + 1 for y in TRAIN_YEARS]  # 2009~2024
TARGET = 'STSelf'
...
model_configs = {
    'STSelf_基准': base_features,
    'STSelf_CEO': ceo_features,
    'STSelf_董事长': chair_features,
}
...
for model_name, features in model_configs.items():
    ...
    for train_year, test_year in zip(TRAIN_YEARS, TEST_YEARS):
        ...
        if len(X_train) < 100 or len(X_test) < 10:
            print(f"警告: {model_name} {train_year}→{test_year} 数据量不足（训练{len(X_train)}，测试{len(X_test)}），跳过")
            skipped_years.append(train_year)
            continue
...
# ==================== 生成汇总报告 ====================
report_lines = []
report_lines.append("STSelf 模型滚动窗口汇总")
report_lines.append("=" * 70)

for model_name in ['STSelf_基准', 'STSelf_CEO', 'STSelf_董事长']:
    info = all_model_summaries[model_name]
    report_lines.append(f"\n【{model_name}】")
    if info['skipped']:
        report_lines.append(f"跳过的训练年（数据不足）: {info['skipped']}")
...
report_path = os.path.join(ROOT, "STSelf_汇总结果.txt")
